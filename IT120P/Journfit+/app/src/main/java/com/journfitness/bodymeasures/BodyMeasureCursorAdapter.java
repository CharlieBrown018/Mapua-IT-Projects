...
public class BodyMeasureCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        @SuppressLint("Range") Date date = DateConverter.DBDateStrToDate(cursor.getString(cursor.getColumnIndex(DAOBodyMeasure.DATE)));
        String dateStr = DateConverter.dateToLocalDateStr(date, mContext);
        t1.setText(dateStr);

        @SuppressLint("Range") float measure = cursor.getFloat(cursor.getColumnIndex(DAOBodyMeasure.MEASURE));
        @SuppressLint("Range") Unit unit = Unit.fromInteger(cursor.getInt(cursor.getColumnIndex(DAOBodyMeasure.UNIT)));
